...
常常被称作“内存卡”或“内存屏障”，其得名原因是它们在代码中**划出界线**，限定某些操作不得通行

栅栏限制了编译器和硬件的不同变量上的先行和同步关系的重新编排
...
fense只有放在两个变量之间, 才会强制这两个操作服从先后次序

#### 互斥实现的原理

`lock()`与 `unlock()`都是某内部内存区域之上的操作，前者是**获取**操作，后者则是**释放**操作



`lock()`的实现方式是在循环中反复调用` flag.text_and_set()`，其中所采用的次序为`std::memory_order_acquire`；`unlock()`实质上则是服从 `std::memory_order_release` 次序的`flag.clear()`操作。



- 给定一互斥对象，在其加锁和解锁的操作序列中，每个 `unlock()`调用都与下一个` lock()`

  调用同步，或与下一个 `try_lock()、try_lock_for()、try_lock_until()`的成功调用同步;但是，如果` try_lock()、try_lock_for()或 try_lock_until()`的调用失败，则不构成任何

  同步关系

- 给定一` std::promise` 对象，则我们由 `get_future()`得到关联的` std:: future` 对象，它们**共享异步状态**。如果 `std::promise` 上的` set_value()`或` set_exception()`调用成功(或者如果出现异常，该异步状态就会存储一个`std:: future_error` 异常对象)又如果我们接着在该 `std::future` 对象上调用 `wait()、get()、wait_for()或 wait_until()`，成功返回`std::future_ status::ready`，那么这两次调用的成功返回(或`std::promise `对象的析构函数与该成功返回)构成同步
