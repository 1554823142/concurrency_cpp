...
public:
    threadsafe_queue() {}
// 更改策略: 将std::shared_ptr<>的初始化移动到push()处, 令队列存储std::shared_ptr<>而不是直接存储数据
/*    threadsafe_queue(threadsafe_queue const& other) {
    ...
    void push(T new_value) {
        std::shared_ptr<T> data(std::make_shared<T>(std::move(new_value)));  //// change 1
    ...
    void wait_and_pop(T& value) {
        ...
        //value = data_queue.front();                               ////////// change 2
    ...
    std::shared_ptr<T> wait_and_pop() {
        ...
        std::shared_ptr<T> res = data_queue.front();                 //////// change 3
        data_queue.pop();
        return res;
    }
    ...
    bool try_pop(T& value) {
        ...
        value = std::move(*data_queue.front());                      //////// change 4
        data_queue.pop();
        return true;
    }
    ...
    std::shared_ptr<T> try_pop() {
        ...
        // std::shared_ptr<T> res(std::make_shared<T>(data_queue.front()));
        std::shared_ptr<T> res = data_queue.front();                //////// change 5
        data_queue.pop();
        return res;
    }
